...
cat ${aishell4_source_dir}/test/TextGrid_list.txt >> ${test_dir}/textgrid.flist
cat ${aishell4_source_dir}/test/wav_list.txt >> ${test_dir}/wav.flist
...
cp $wav_list_aishell4 $train_dir/
cp $text_grid_aishell4 $train_dir/
...
cat $train_dir/text_all | perl local/text_normalize.pl | perl local/text_format.pl | sort -u > $train_dir/text
...
cat $test_dir/text_all | perl local/text_normalize.pl | perl local/text_format.pl | sort -u > $test_dir/text
